...
import deluxe from "@/assets/room-deluxe.webp.asset.json";
import twin from "@/assets/room-twin.webp.asset.json";
import single from "@/assets/room-single.webp.asset.json";
import { Reveal } from "./Reveal";

export function Rooms() {
  const { tr } = useI18n();
  const rooms = [
    { img: deluxe.url, t: tr("room.deluxe.t"), d: tr("room.deluxe.d"), label: "Deluxe Room" },
    { img: twin.url, t: tr("room.twin.t"), d: tr("room.twin.d"), label: "Standard Twin" },
    { img: single.url, t: tr("room.single.t"), d: tr("room.single.d"), label: "Double Room" },
  ];
          ...
          {rooms.map((r, i) => (
            <Reveal key={r.t} delay={i * 0.1}>
              <div className="group">
                ...
                <p className="text-washi/55 text-sm leading-relaxed">{r.d}</p>
              </div>
            </Reveal>
          ))}
        </div>
